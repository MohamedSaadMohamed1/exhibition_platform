...
exports.onNewMessage = functions.firestore
    ...
    if (((_a = recipientDoc.data()) === null || _a === void 0 ? void 0 : _a.activeChat) === chatId) {
...
exports.onUserCreated = functions.firestore
    ...
    if (!adminSnapshot.empty) {
        await Promise.all(adminSnapshot.docs.map((doc) => sendPushNotification({
            userId: doc.id,
...
exports.onAccountApprovalStatusChanged = functions.firestore
    .document('account_requests/{requestId}')
    .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    if (before.status === after.status || !after.userId)
        return;
...
exports.onJobApplicationCreated = functions.firestore
    ...
    if (!job || !job.organizerId)
        return;
    const applicantName = ((_a = applicantDoc.data()) === null || _a === void 0 ? void 0 : _a.name) || 'Someone';
    await sendPushNotification({
...
//   }
exports.setRoleClaimsOnUserWrite = functions.firestore
    .document('users/{userId}')
    ...
    const role = (_a = data.role) !== null && _a !== void 0 ? _a : 'visitor';
    const isActive = (_b = data.isActive) !== null && _b !== void 0 ? _b : true;
